...
def inference(data_loader, model, result_path, class_names, no_average,
    ...
    with torch.no_grad():
        for i, (inputsrgb, inputshtg, targets) in enumerate(data_loader):
            data_time.update(time.time() - end_time)

            video_ids, segments = zip(*targets)
            outputsrgb, outputshtg = model(inputsrgb, inputshtg)
            outputs = F.softmax(outputsrgb + outputshtg, dim=1).cpu()
